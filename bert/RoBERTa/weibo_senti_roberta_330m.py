# %% [markdown]
# # 微博情感分析 —— RoBERTa-330M 微调
# 基于 `Fengshenbang/Erlangshen-RoBERTa-330M-Sentiment` 对微博评论进行二分类情感分析（正面 / 负面）
# 使用 ModelScope 库加载模型
...
# %% 4. 通过 ModelScope 下载并加载 Tokenizer 与模型
if os.path.exists(LOCAL_MODEL_PATH):
    print(f"检测到本地模型: {LOCAL_MODEL_PATH}")
    model_dir = LOCAL_MODEL_PATH
else:
    print(f"本地模型不存在，正在从 ModelScope 下载: {MODEL_NAME}...")
